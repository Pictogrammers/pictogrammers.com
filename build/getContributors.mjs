...
const getContributors = async () => {
  ...
  try {
    ...
    if (!contributors.length) {
      ...
      return fs.writeFileSync('../client/public/data/contributors.json', JSON.stringify({ contributors: [], totalContributors: 0 }), { flag: 'w' });
    }
    ...
    const processedContributors = contributors
      .reduce((output, contributor) => {
        ...
        if (base64 !== genericImage) {
          ...
          fs.writeFileSync(`../client/public${imagePath}`, contributor.base64, { encoding: 'base64', flag: 'w' });
          rest.image = true;
        }
    ...
    fs.writeFileSync('../client/public/data/contributors.json', JSON.stringify({ contributors: processedContributors, totalContributors: processedContributors.length }), { flag: 'w' });
