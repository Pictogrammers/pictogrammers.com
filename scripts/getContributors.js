...
const getContributors = async () => {
  ...
  try {
    ...
    const processedContributors = contributors
      .map((contributor) => {
        ...
        if (base64 !== genericImage) {
          ...
          fs.writeFileSync(`./public${imagePath}`, contributor.base64, { encoding: 'base64' });
          rest.image = true;
        }
    ...
    return fs.writeFileSync('./public/contributors/contributors.json', JSON.stringify({ contributors: processedContributors, totalContributors: processedContributors.length }), { flag: 'w' });
  } catch (err) {
    console.error(err);
    process.exit(1);
  }
