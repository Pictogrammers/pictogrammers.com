      ...
      const libraries = await Promise.all(
        Object.keys(allLibraries).map(async (libraryId) => {
          ...
          if (dbIconCount < count) {
            ...
            // Empty the mismatched tables
            await Promise.all([
              db.table('icons').clear(),
              db.table('tags').clear()
            ]);
  
            // Load 'em up with data
            await Promise.all([
              db.table('icons').bulkPut(icons),
              db.table('tags').bulkPut(tags.map((tag: string, i: number) => ({ id: i, name: tag, slug: slugify(tag, { lower: true }) })))
            ]);
